...
対局形式は `RuleConfig.gameLength`（`tonpuu`=東風戦＝東4局 / `hanchan`=半荘＝南4局、既定 `hanchan`）。終局条件は次の優先で `startNextHand` が判定する:

1. **トビ**: `RuleConfig.tobiThreshold`（既定0）を下回った者が出たら即終局。`null` でトビなし。
2. **アガリやめ／テンパイやめ**（`RuleConfig.agariyame`、既定 on）: オーラスで親が連荘条件を満たし（和了 or 親聴牌流局。途中流局は除く）、かつ親がトップなら終局。off なら延長戦（同じ局位置で本場+1）。
3. **オーラスで親流れ**: 終局。

順位は点数降順、同点は起家に近い席優先（既定）。`finalRanking(state)` が確定し、局送りは `startNextHand(state): NextHand`（`{over:true, ranking}` か `{over:false, state}`）。

## 実装状況（[ADR-0004](../decisions/0004-ruleset-riichi-full.md)）

Phase 1〜4b すべて実装済み: 配牌・ツモ・打牌・リーチ・フリテン（河/同巡/リーチ後恒久）・一発・海底/河底・鳴き全種（チー/ポン/大明槓/暗槓/加槓）・槍槓・嶺上ツモ・カンドラ・喰い替え防止・送り槓・各種流局・流し満貫・点移動・包（大三元/大四喜の責任払い、[yaku-scoring](yaku-scoring-design.md)）・連荘/親送り/本場/供託・終局（東風/半荘・アガリやめ・トビ）/順位。CPU は `chooseAction`（[architecture](architecture.md#cpu対戦相手aiの置き場所)）。
