...
包（責任払い）: 確定鳴き（ポン/大明槓）で**大三元・大四喜**を確定させた牌の放出者を責任者として記録（`GameState.pao[winner]`）。和了役満に対象役が含まれるときのみ発動し、その役満分を pao分（責任者負担）と free分（通常分配）に分ける。
- ツモ: pao分は責任者が全額（3家分）を負担。free分は通常のツモ分配。
- ロン: pao分は責任者と放銃者で折半（責任者＝放銃者なら全額）。free分は放銃者が全額。
- 本場・供託は通常通り（本場はツモ=各非和了者/ロン=放銃者、供託は和了者へ）。手変わりで対象役満が消えたら通常分配にフォールバック。

検証は「点数早見表」との突合テスト（`core/tests/scoring.*.test.ts`）で固定する。境界（30符4翻=7700/満貫、20符などの切り上げ）を必ずテストに入れる。

## 未確定・ローカルルール（既定値、変更時は ADR 追加）

- 切り上げ満貫: なし / 流し満貫: あり / 人和: 未採用 / 連風牌雀頭: 4符 / ダブル役満: あり / 包（大三元・大四喜）: あり。
